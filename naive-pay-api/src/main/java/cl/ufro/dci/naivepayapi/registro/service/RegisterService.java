...
public class RegisterService {
    ...
    public void startRegistration(RegistrationStartDTO startData) {
        ...
        Register existingRegister = registerRepository.findByEmail(startData.getEmail());
        if (existingRegister != null && existingRegister.isVerified()) {
        ...
        register.setEmail(startData.getEmail());
        register.setHashedLoginPassword(hashedPassword);
        register.setRegisterDate(new Date());
        register.setVerified(false);
        register.setVerificationCode(verificationCode);
        register.setVerificationCodeExpiration(expirationDate);
    ...
    public void verifyEmail(String email, String code) {
        ...
        if (register.getVerificationCodeExpiration().before(new Date())) {
            throw new IllegalStateException("El código de verificación ha expirado.");
        }

        // Se valida el codigo de verificacion
        if (register.getVerificationCode() == null || !register.getVerificationCode().equals(code)) {
    ...
    public void resendVerificationCode(String email) {
        ...
        register.setVerificationCode(newCode);
        register.setVerificationCodeExpiration(newExpirationDate);
        registerRepository.save(register);

        // Enviar el nuevo código por email
        emailService.sendVerificationEmail(register.getEmail(), newCode);
    }
    ...
    public User completeRegistration(RegistrationCompleteDTO completeData) {
        ...
        Register register = registerRepository.findByEmail(completeData.getEmail());

        if (userRepository.findByRutGeneral(completeData.getRutGeneral()).isPresent()) {
        ...
        try {
            ...
            credencial.setPrivateKeyRsa(privateKeyStr);
            credencial.setPublicKeyRsa(publicKeyStr);
    ...
    private Credencial createDefaultCredencial() {
        ...
        credencial.setCreationDate(new Date());
        credencial.setDenied(false);
        credencial.setActiveDinamicKey(true);
        return credencial;
    }
    ...
    private User buildUserFromDTO(RegistrationCompleteDTO userData) {
        User user = new User();
        user.setNames(userData.getNames());
        user.setLastNames(userData.getLastNames());
        user.setRutGeneral(userData.getRutGeneral());
        user.setVerificationDigit(userData.getVerificationDigit());
