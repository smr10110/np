...
public class DeviceLog {
    ...
    /** ID of the user associated with the device action */
    ...
    private void ensureUserIdPresent() {
        if (this.userId == null) {
            if (this.device != null && this.device.getUser() != null && this.device.getUser().getUseId() != null) {
                this.userId = this.device.getUser().getUseId();
            } else {
                throw new IllegalStateException("DeviceLog requiere userId o device.user.useId");
            }
        }
    }
