...
public class AccountLockService {
    ...
    public boolean checkAndBlockIfNeeded(User user) {
        ...
        if (isAccountLocked(user)) {
            logger.debug("Usuario {} ya está bloqueado, omitiendo verificación", user.getId());
            return false;
        }
        ...
        if (recentAttempts.size() < maxFailedAttempts) {
            logger.debug("Usuario {} tiene solo {} intentos, necesita {} para bloquear",
                    user.getId(), recentAttempts.size(), maxFailedAttempts);
            return false;
        }

        // Condición 2: Todos deben ser fallidos (sin éxitos intercalados)
        if (!areAllAttemptsFailed(recentAttempts)) {
            logger.debug("Usuario {} tiene intentos exitosos entre los recientes, no se bloquea", user.getId());
            return false;
        }

        // Condición 3: Deben estar dentro de la ventana temporal
        if (!areAttemptsWithinLockoutWindow(recentAttempts)) {
            logger.debug("Los intentos fallidos del usuario {} están fuera de la ventana temporal, no se bloquea", user.getId());
            return false;
        }
        ...
        logger.info("Bloqueando cuenta del usuario {} debido a {} intentos fallidos en {} minutos",
                user.getId(), maxFailedAttempts, lockoutWindowMinutes);
     ...
     * @throws IllegalArgumentException si {@code user} es {@code null}
     * @throws IllegalStateException si {@code user.getId()} es {@code null}
        ...
        try {
            ...
            getUserEmail(user).ifPresentOrElse(
                    email -> {
                        ...
                        logger.debug("Email de notificación de bloqueo enviado al usuario {}", user.getId());
                    },
                    () -> logger.debug("No se envió email al usuario {}: no tiene dirección de email válida", user.getId())
            );
    ...
    private void logAccountBlocked(User user) {
        ...
        MDC.put("rut", user.getRutGeneral() + "-" + user.getVerificationDigit());
