...
public class PasswordRecoveryService {
    ...
    private void logPasswordReset(User user) {
        MDC.put("userId", String.valueOf(user.getUseId()));
        MDC.put("email", user.getRegister().getRegEmail());
        MDC.put("accountUnblocked", String.valueOf(user.getUseState() == AccountState.ACTIVE));
        try {
            logger.info("Contraseña actualizada exitosamente | userId={} | email={} | cuentaDesbloqueada={}",
                user.getUseId(), user.getRegister().getRegEmail(), user.getUseState() == AccountState.ACTIVE);
        } finally {
            MDC.remove("userId");
            MDC.remove("email");
