...
public class PasswordRecoveryService {
    ...
    public void sendRecoveryCode(String email) {
        var userOpt = userRepository.findByRegisterRegEmail(email);
        if (userOpt.isEmpty()) {
        ...
        invalidatePendingCodes(user.getUseId());
    ...
    public void resetPassword(String email, String code, String newPassword) {
        ...
        user.getRegister().setRegHashedLoginPassword(passwordEncoder.encode(newPassword));
        recovery.setPasStatus(PasswordRecoveryStatus.USED);
        recovery.setPasUsed(Instant.now());

        // Desbloquea cuenta si estaba bloqueada
        boolean wasBlocked = user.getUseState() == AccountState.INACTIVE;
        if (wasBlocked) {
            user.setUseState(AccountState.ACTIVE);
        }

        // Registrar intento exitoso de PASSWORD_RESET para reiniciar contador de fallos
        registerPasswordResetAttempt(user);

        // Email de confirmación
        sendPasswordChangeEmail(email, user.getUseNames());

        logPasswordReset(user, wasBlocked);
    }
    ...
    private PasswordRecovery validateRecoveryCode(String email, String code) {
        User user = userRepository.findByRegisterRegEmail(email)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "INVALID_CODE"));

        PasswordRecovery recovery = passwordRecoveryRepository.findByUser_UseIdAndPasCode(user.getUseId(), code)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "INVALID_CODE"));
    ...
    // === MÉTODOS HELPER: AuthAttempt ===

    /**
     * Registra un intento exitoso de PASSWORD_RESET en AuthAttempt.
     * Esto reinicia el contador de intentos fallidos del usuario.
     *
     * @param user Usuario que reseteó su contraseña
     */
    private void registerPasswordResetAttempt(User user) {
        deviceService.findByUserId(user.getUseId())
                .ifPresentOrElse(
                    device -> {
                        authAttemptService.log(device, true, AuthAttemptReason.PASSWORD_RESET);
                        logger.info("AuthAttempt PASSWORD_RESET registrado | userId={} | deviceFingerprint={}",
                            user.getUseId(), device.getFingerprint());
                    },
                    () -> logger.warn("No se registró PASSWORD_RESET: usuario sin dispositivo | userId={}",
                        user.getUseId())
                );
    }
     ...
     * @param wasBlocked Si la cuenta estaba bloqueada antes del reset
     */
    private void logPasswordReset(User user, boolean wasBlocked) {
        MDC.put("userId", String.valueOf(user.getUseId()));
        MDC.put("email", user.getRegister().getRegEmail());
        MDC.put("accountUnblocked", String.valueOf(wasBlocked));
        try {
            logger.info("Contraseña actualizada exitosamente | userId={} | email={} | cuentaDesbloqueada={}",
                user.getUseId(), user.getRegister().getRegEmail(), wasBlocked);
        } finally {
            MDC.remove("userId");
            MDC.remove("email");
